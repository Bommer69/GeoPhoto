    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-xl text-blue-200">Đang tải album...</p>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900 flex items-center justify-center p-4">
        <div className="bg-white/10 backdrop-blur-lg rounded-2xl p-8 text-center max-w-md border border-red-500/30">
          ...
          <h3 className="text-xl font-bold text-white mb-2">Không tìm thấy album</h3>
          <p className="text-gray-300 mb-6">{error || 'Album không tồn tại hoặc đã bị xóa.'}</p>
          <button
      ...
      <header className="sticky top-0 z-50 bg-slate-900/80 backdrop-blur-lg border-b border-white/10">
        <div className="max-w-7xl mx-auto px-4 py-4">
              ...
              {editMode ? (
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="px-3 py-1.5 bg-white/10 border border-white/20 rounded-lg text-white focus:outline-none focus:border-blue-500"
                    autoFocus
                  />
                  <button
          ...
          {album.description && !editMode && (
            <p className="text-sm text-blue-300/70 mt-2 max-w-2xl">{album.description}</p>
          )}
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-4 py-6">
        ...
        {(!album.photos || album.photos.length === 0) && (
          <div className="text-center py-20">
            ...
            <h3 className="text-2xl font-bold text-white mb-2">Album trống</h3>
            <p className="text-blue-300 mb-6">Thêm ảnh vào album từ thư viện hoặc bản đồ.</p>
            <button
              onClick={() => navigate('/library')}
              className="px-6 py-3 bg-gradient-to-r from-blue-500 to-cyan-500 text-white rounded-xl hover:from-blue-600 hover:to-cyan-600 transition font-medium"
            >
              Đến Thư viện ảnh
            </button>
          </div>
        ...
        {album.photos && album.photos.length > 0 && (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
            {album.photos.map((photo) => (
              <div
                key={photo.id}
                className="group relative aspect-square rounded-xl overflow-hidden bg-white/5 cursor-pointer transform hover:scale-105 transition-all duration-300"
              ...
              >
                ...
                {album.coverPhotoId === photo.id && (
                  <div className="absolute top-2 left-2 px-2 py-1 bg-blue-500/90 text-white text-xs font-medium rounded-full">
                    Ảnh bìa
                  </div>
                )}
                ...
                <div className={`absolute top-2 right-2 px-2 py-1 rounded-full text-xs font-medium ${
                  ...
                  {photo.latitude && photo.longitude ? '📍' : '⚠️'}
                </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <div className="bg-slate-800 rounded-2xl p-6 max-w-md w-full border border-white/10 shadow-2xl">
            <div className="text-center">
              ...
              <h3 className="text-xl font-bold text-white mb-2">Xóa ảnh khỏi album?</h3>
              <p className="text-gray-300 mb-1">
                Ảnh "<span className="text-orange-400">{removeConfirm.fileName}</span>" sẽ được xóa khỏi album này.
              </p>
